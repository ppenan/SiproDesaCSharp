...
namespace SiproDAO.Dao
{
    public class CategoriaAdquisicionDAO
    {
        ...
        public static long getTotalCategoriaAdquisicion(String filtro_busqueda)
        {
            long ret = 0L;
            try
            {
        ...
        public static List<CategoriaAdquisicion> getCategoriaAdquisicionPagina(int pagina, int numeroCategoriaAdquisicion, String filtro_busqueda, String columna_ordenada, String orden_direccion)
        {
            ...
            try
            {
                using (DbConnection db = new OracleContext().getConnection())
                {
                    ...
                    if (filtro_busqueda != null && filtro_busqueda.Length > 0)
                    {
                        ...
                        if (DateTime.TryParse(filtro_busqueda, out fecha_creacion))
                        {
                            query_a = String.Join(" ", query_a, (query_a.Length > 0 ? " OR " : ""), " TO_DATE(TO_CHAR(ca.fecha_creacion,'DD/MM/YY'),'DD/MM/YY') LIKE TO_DATE('" + fecha_creacion.ToString("dd/MM/yyyy") + "','DD/MM/YY') ");
                        }
                    }
